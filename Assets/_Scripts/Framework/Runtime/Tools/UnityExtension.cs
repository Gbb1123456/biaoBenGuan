...
namespace ZXKFramework
{
    public static class UnityExtension
    {
        ...
        public static GameObject FindFirst(this Transform transform, string objName)
        {
            return FindGameobject(transform, objName);
        }
        public static GameObject FindFirst(this GameObject gameObject, string objName)
        {
            return FindGameobject(gameObject.transform, objName);
        }
        public static T FindFirst<T>(this Transform transform, string objName) where T : Component
        {
            return FindGameobject<T>(transform, objName);
        }
        public static T FindFirst<T>(this GameObject gameObject, string objName) where T : Component
        {
            return FindGameobject<T>(gameObject.transform, objName);
        }

        //查找组件
        public static T FindGameobject<T>(Transform trs, string objName) where T : Component
        {
            GameObject tempObj = FindGameobject(trs, objName);
            if (tempObj != null)
            {
                if (tempObj.GetComponent<T>() != null)
                {
                    return tempObj.GetComponent<T>();
                }
                else
                {
                    Debug.LogError(string.Format("FindTools.FindGameobject 无法找到 {0} 对应的游戏组件 {1}", objName, typeof(T).Name));
                }
            }
            return null;
        }
        ...
        //所有提示都已写全
        public static GameObject FindGameobject(Transform trs, string objName)
        {
            if (trs == null)
            {
                Debug.LogError("FindTools.FindGameobject trs = null");
                return null;
            }
            if (string.IsNullOrEmpty(objName))
            {
                Debug.LogError("FindTools.FindGameobject objName = null");
                return null;
            }
            GameObject tempObj = Find(trs, objName);
            if (tempObj == null)
            {
                Debug.LogError(string.Format("FindTools.FindGameobject 无法找到 {0} 对应的游戏物体", objName));
            }
            return tempObj;
        }

        private static GameObject Find(Transform trs, string objName)
        {
            if (trs.name == objName)
            {
            ...
            if (trs.childCount > 0)
            {
                ...
                {
                    GameObject obj = Find(tempTrs, objName);
                    if (obj != null)
                    {
                        return obj;
                    }
